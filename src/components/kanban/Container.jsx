...
const Container = ({ id, children, title, onAddItem }) => {
	...
	return (
		<div
			...
			style={style}
			ref={setNodeRef}
			className={`w-full min-h-32 h-full p-2 shadow-md bg-gray-50 rounded-sm flex flex-col gap-y-2 ${isDragging && 'opacity-50'}`}
		>
			<div className='flex items-center justify-between'>
				<div className='flex flex-col gap-y-1 h-min'>
					<h1 className='px-2 text-lg font-semibold text-slate-700'>{title}</h1>
				</div>
				<button
			...
			<button
				onClick={onAddItem}
				className='text-slate-700 font-medium mx-auto px-2 py-1 rounded-sm duration-300 hover:bg-slate-100 hover:shadow-md transition w-max'
			>
				Add item
			</button>
		</div>
